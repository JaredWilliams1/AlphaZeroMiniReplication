...
def gen_batch(model, batch_save_path, batch_size):  # 4.48% residual time  |  96.22% of total time
    tuple_list = []
    for game_num in range(batch_size):
        tuple_list.append(tree_record(model, Game().gameState))     # cont: 91.74% of total time
        print("GAME:", game_num)
    with open(batch_save_path, "wb") as file:
...
def tree_record(model, gamestate):  # 0.75% residual time  |  91.74% of total time
    ...
    move_list = []
    winner = 0
    game_list.append(tree.root.proper_board[0])
    ...
    while True:
        ...
        if move_count < 5:  # this is for exploration in the first 5 moves
            visit_sum = np.sum(tree.root.children_visits)
            prop = tree.root.children_visits / visit_sum
            max_index = np.random.choice(np.arange(7), p=prop)
        else:  # this is after, the nn plays it's best move
            max_index = np.where(tree.root.children_visits == np.amax(tree.root.children_visits))[0][0]
        move_count += 1
        ...
        if game.gameState.isEndGame:
            ...
            game_list.append(tree.root.proper_board[0])
            move_list.append(tree.root.nn_prediction[0])
    full_tuple = (np.array(game_list), np.array(winner), np.array(move_list))
    return full_tuple


def data_gen_testing(number):  # 4.48% residual time  |  96.22% of total time
    tuple_list = []
    game_count = 0
    ...
    for y in range(number):
        tuple_list.append(tree_record_testing(model, Game().gameState))     # cont: 91.74% of total time
        game_count += 1
        print("GAME:", game_count)
...
def tree_record_testing(model, gamestate):  # 0.75% residual time  |  91.74% of total time
    game = Game()
    game.gameState = gamestate
    tree = Tree(model, gamestate_start=gamestate)
    game_list = []
    ...
    while True:
        ...
        if game.gameState.isEndGame:
            ...
            print(tree.root.nn_prediction[0])
            print(tree.root.nn_prediction[1])
            sys.exit()
            game_list.append(tree.root.proper_board[0])
            move_list.append(tree.root.nn_prediction[0])
